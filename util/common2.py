...
def logos() -> tuple[Image.Image, Image.Image]:
...
def merge_sections_to_divisions(geo, df_section, language):
    if language == "a":
        sections = df_section.division_a
    elif language == "e":
        sections = df_section.division_e
    else:
    ...
    for i, name in enumerate(sections):
        polygons = []
        to_combine = [f for f in geo["features"] if f["properties"][f'division_{language}']==name]
        ...
        if language == "a":
            new_feature = dict(type='Feature', id=i, properties=dict(division_a=name),
                            geometry=dict(type=new_geometry['type'], 
                                        coordinates=new_geometry['coordinates']))
        else:
            new_feature = dict(type='Feature', id=i, properties=dict(division_e=name),
                            geometry=dict(type=new_geometry['type'], 
                                        coordinates=new_geometry['coordinates']))
...
def make_folium_choropleth(geo, indicator, df, col_name, lang):
    # aliases
    division_alias = {
        "a": "القسم:", "e": "Division:"
    }
    ...
    if 'section' in df.columns:
        fields = [f'division_{lang}', col_name, indicator]
        aliases = [division_alias[lang], section_alias[lang], f"{ylable}:"]
        geo["ch_name"] = "Gezira Sections"
    else:
        fields = [col_name, indicator]
        aliases = [division_alias[lang], f"{ylable}:"]
...
def make_alt_linechart(df, indicator, col_name, season, selected_section, language):
    ylable, text = indicator_title(indicator, stat_dict)
    ...
    if selected_section is not None:
        ...
        df_sorted = move_rows_to_top(df, f'section_{language}', selected_section).iloc[::-1]
...
def create_folium_map(data, geo, bounds, crs, variable, lang):
    ...
    geo_layer = add_geojson_layer(geo, folium_map, lang)
...
def plotly_pie_chart(dfca, name, year, lang):
    land_use_type_ar = {
    ...
    fig.update_traces(hole=.3, textposition='inside', textinfo='percent', textfont_size=16)
